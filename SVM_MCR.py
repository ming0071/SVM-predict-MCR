...
def main():
    ...
    # Load dataset
    MCR_excel = "MMH-MM-MERGED-0614.xlsx"
    ...
    if all_feature_mode is True:
        ...
        df = hd_parm.delete_unnecessary_data(df, max_allowed_nan_per_row=15)
        df = hd_parm.impute_nan(
            df, method="bayesian_ridge", max_allowed_nan_per_column=0
        )
        df = hd_parm.process_disease_features(df)
    else:
        # only select best feature(SELECT_FEATURES) to avoid overfitting
        df = hd_parm.process_disease_features(df)
        df = hd_parm.select_features(df)
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        df, Y, test_size=0.2, random_state=5
    )
    print("Original :")
